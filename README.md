...
- Backend: `cd backend && pytest` — **17 passed** (signup/login/me, password rules + name max 100, refresh rotation + reuse→all-sessions-revoked, expired-access 401→refresh→retry, non-member 403 sweep across project/task/comment/activity endpoints, member task-management vs membership-denial, remove-member preserves tasks, project-delete cascade with zero orphans, logout, SQLite naive-datetime, invite/permissions, Done rule + `completed_at`, due/assignee 422s, delete rule, pagination/filters/search, comments, health/ready, envelope shape).
